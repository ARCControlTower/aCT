...
class aCTSubmitter(aCTCondorProcess):
    ...
    def submit(self):
        ...
        - The signals are deferred per fairshare.

        - TODO: Review possible interrupt handling strategies for interaction
                with Condor cluster.
        """
        if not self.cluster:
    ...
    def checkFailedSubmissions(self):
        """
        Update jobs that failed to be submitted.
    ...
    def processToResubmit(self):
        """
        Clean jobs from Condor and mark them for submission.
