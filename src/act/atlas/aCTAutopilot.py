...
class aCTAutopilot(aCTATLASProcess):
    ...
    def updatePandaHeartbeat(self,pstatus):
        ...
        with self.db.Session() as session:
            jobs = session.execute(select(PandaJob.pandaid, PandaJob.siteName, PandaJob.startTime,
                                        PandaJob.computingElement, PandaJob.node, PandaJob.corecount)
                                    .where(PandaJob.pandastatus==pstatus, PandaJob.sendhb==1,
                                           or_((PandaJob.modified > PandaJob.theartbeat),
                                               self.db.timeStampLessThan(PandaJob.theartbeat, self.conf.panda.heartbeattime))).limit(1000)).all()
        if not jobs:
            return
            ...
            for t in tlist:
                ...
                for pandaid, response in zip(t.ids, t.result[1]):
                    ...
                    if result.get('command', [''])[0] in ["tobekilled", "badattemptnr", "alreadydone"]:
                        ...
                        jd['actpandastatus'] = "tobekilled"
                        jd['pandastatus'] = None
                    session.execute(update(PandaJob).where(PandaJob.pandaid==pandaid).values(**jd))
        ...
        with self.db.Session.begin() as session:
            for j in jobs:
                ...
                if not j.sendhb:
                    ...
                    if not j.startTime:
                        jd['startTime'] = self.db.getTimeStamp()
                    if not j.endTime:
                        jd['endTime'] = self.db.getTimeStamp()
                    session.execute(update(PandaJob).where(PandaJob.pandaid==j.pandaid).values(**jd))
                    continue
                ...
                if j.actpandastatus == 'cancelled':
                    ...
                    jobinfo.startTime = j.startTime if j.startTime else self.db.getTimeStamp()
                    jobinfo.endTime = j.endTime if j.endTime else self.db.getTimeStamp()
                else:
                    try:
    ...
    def checkJobs(self):
        ...
        with self.db.Session.begin() as session:
            jobs = session.execute(select(PandaJob.pandaid, PandaJob.pandastatus).where(PandaJob.pandastatus.is_not(None))).all()
    ...
    def updateArchive(self):
        ...
        with self.db.Session() as session:
            jobs = session.execute(select(PandaJob.pandaid, PandaJob.siteName, PandaJob.actpandastatus, PandaJob.startTime, PandaJob.endTime, PandaJob.modified) \
                                   .where(self.db.timeStampLessThan(PandaJob.modified, 60*60*24), PandaJob.actpandastatus.in_(['done', 'donefailed', 'donecancelled']))).all()
        if not jobs:
            return
        ...
        with self.db.Session.begin() as session:
            for job in jobs:
                jobarchive = job._asdict()
                self.stopOnFlag()
                self.log.info(f"Archiving panda job appjob({job.pandaid})")
                # Fill out empty start/end time
                if job.startTime:
                    if not job.endTime:
                        jobarchive['endtime'] = job.modified
                elif job.endTime:
                    jobarchive['starttime'] = job.endTime
                else:
                    jobarchive['starttime'] = tstamp
                    jobarchive['endtime'] = tstamp

                # archive table doesn't have modified
                del jobarchive['modified']
                session.execute(insert(PandaArchive).values(**jobarchive))
                session.execute(delete(PandaJob).where(PandaJob.pandaid==job.pandaid))
